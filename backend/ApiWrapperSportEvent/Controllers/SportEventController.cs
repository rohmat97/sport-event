...
public class SportEventController : ControllerBase
{
    ...
    public SportEventController(IHttpClientFactory httpClientFactory, ILogger<UserController> logger)
    {
    ...
    [HttpGet]
    public async Task<ActionResult<RootSportEvent>> GetAllSportEvent([FromHeader(Name = "Authorization")] string bearerToken)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(bearerToken))
            {
                ...
                var response = await httpClient.GetAsync("/api/v1/sport-events");
                _logger.LogInformation(response.ToString());
                // Check if the request was successful
                if (response.IsSuccessStatusCode)
                {
                    // Deserialize the JSON response to User
                    var userDetail = await response.Content.ReadFromJsonAsync<RootSportEvent>();
